...
readData(__dirname).forEach((str) => {
  ...
  const twoSameCardsCount = Object.values(cardMap).filter(
    (num) => num === 2
  ).length;

  const maxOfKind = Math.max(...Object.values(cardMap)) + jokers;

  let weight;

  if (maxOfKind === 5 || jokers === 5) {
    weight = 7;
  } else if (maxOfKind === 4) {
    weight = 6;
  } else if (maxOfKind === 3 && twoSameCardsCount > jokers ? 1 : 0) {
    weight = 5;
  } else if (maxOfKind === 3) {
    weight = 4;
  } else if (maxOfKind === 2 && twoSameCardsCount > 1) {
    weight = 3;
  } else if (maxOfKind === 2) {
    weight = 2;
  } else {
